...
const Feedback = async ({ params }: RouteParams) => {
  ...
  // 🔥 FIX: normalize categoryScores (object -> array safe conversion)
  const categoryScoresArray = feedback?.categoryScores
    ? Array.isArray(feedback.categoryScores)
      ? feedback.categoryScores
      : Object.entries(feedback.categoryScores).map(([name, value]: any) => ({
          name,
        ...
        {categoryScoresArray.length > 0 ? (
          categoryScoresArray.map((category: any, index: number) => (
            <div key={index}>
              ...
              <p>{category.comment || "No comments provided."}</p>
            </div>
          ))
        ) : (
